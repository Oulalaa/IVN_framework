//——————————————————————————————————————————————————————————————————————————————
//  ACAN2515 Demo in normal mode,ECU4
//——————————————————————————————————————————————————————————————————————————————
...
//  Serial.println ("Configure ACAN2515") ;
  ...
  const ACAN2515AcceptanceFilter filters [] = {
    {extended2515Filter (0x444D444), receive0},
  } ;
...
CANMessage inmsg; 
  if (can.available ()) {
   can.receive(inmsg);
     Serial.println();
      Serial.print ("Receive4Frame_id: ") ;
     Serial.print(inmsg.id,HEX);
     Serial.print ("    Received4_SentFrameCount: ") ;
