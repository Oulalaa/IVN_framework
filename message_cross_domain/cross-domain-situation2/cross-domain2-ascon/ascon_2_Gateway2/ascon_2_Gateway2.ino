...
//--- Begin SPI
  ...
  const ACAN2515AcceptanceFilter filters [] = {
    {extended2515Filter (0x4200041), receive0},
    {extended2515Filter (0x8280021), receive0},
  }; 
   ...
   if(can.available()){
    ...
    if(frame_cipher_or_rtr.rtr==true){
      ...
      if((frame_cipher_or_rtr.id&0x1c1800e0)==0x4000040){
        frame_cipher_or_rtr.id = 0x7ee0041;
        ...
        frame_cipher_or_rtr.id = 0Xbf60021;
        frame_mac.id = frame_cipher_or_rtr.id;
        //----------------------------------send_RTR-------------------------------------
        const bool ok_cipher_or_rtr = can.tryToSend (frame_cipher_or_rtr) ;
        const bool ok_mac = can.tryToSend (frame_mac) ;
    ...
    //------------------------------------------------Receive DATA----------------------------------------------
       ...
       if((frame_cipher_or_rtr.id&0x1c1800e0)==0x8080020){
        ...
        if(same_mac){
          ...
          const bool ok_cipher_or_rtr = can.tryToSend (frame_cipher_or_rtr) ;
          if (ok_cipher_or_rtr) {
            Serial.println ("Send cipher success") ;
          }else{
            Serial.println ("Send cipher failure") ;
          }
          //----------------------------------send_mac-------------------------------------   
          const bool ok_mac = can.tryToSend (frame_mac) ;
          if (ok_mac) {
            Serial.println ("Send mac success") ;
          }else{
            Serial.println ("Send mac failure") ;
          }
