...
//--- Begin SPI
  ...
  const ACAN2515AcceptanceFilter filters [] = {
    ...
    {extended2515Filter (0x0160021), receive0},//跨域RTR 0x8300121
    {extended2515Filter (0x0128042), receive0},//域内RTR 0x8300142
    {extended2515Filter (0x0060041), receive0},//跨域DATA 0x4200141
    {extended2515Filter (0x0028041), receive0},//域内DATA 0x8400141
  }; 
...
void chaskey2_2(CANMessage *frame,CANMessage *frame_mac){
   // ——————————————Chaskey8 init——————————————
    uint8_t tag[8];
   uint32_t k[4] = {0x833D3433, 0x009F389F, 0x2398E64F, 0x417ACF39}; //Key used
   uint32_t k1[4], k2[4];  //Subkeys used
...
//------------------------------------------------Sent RTR----------------------------------------------
    ...
    case 0 : // send RTR kuayu (uno1)
     ...
     case 1 : // send RTR bu kua yu(uno4)
      frame.id = 0Xbf28041 ;
      frame_cipher_or_rtr.id = frame.id;
      frame_mac.id = frame.id;
...
//------------------------------------------------Receive RTR----------------------------------------------
    if(frame_cipher_or_rtr.rtr==true){
      if(frame_cipher_or_rtr.id==0Xbf60021){
        frame_cipher_or_rtr.id = 0x4260041;
      ...
      else if(frame_cipher_or_rtr.id==0Xbf28042){
        ...
        Serial.println ("Receive RTR from uno4") ;
        Serial.println ("-----------------Receive RTR success---------------") ;
      }
      chaskey2_2(&frame,&frame_mac);
